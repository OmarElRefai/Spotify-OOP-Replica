...
public class Library
{
	...
	public void download(AudioContent content)
	{
		if (content.getType().equals(Song.TYPENAME))
		{
			if (songs.contains(content))
			{
				...
				throw new AudioContentAlreadyDownloadedException(content.getType() + " " + content.getTitle() + " already downloaded");
			}
	...
	public void playPlaylist(String playlistTitle, int index)
	{
		...
		if (index < 1 || index > pl.getContent().size())
		{
			...
			throw new PlaylistNotFoundException("Invalid Playlist AudioContent");
		}
	...
	static class AudioContentNotFoundException extends RuntimeException {
	...
	static class GenreNotFoundException extends RuntimeException {
